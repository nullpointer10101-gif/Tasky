...
export default function Referral({ user }) {
  ...
  const handleShare = () => {
    if (refData?.referral_link && window.Telegram?.WebApp) {
      const text = `Join Tasky and earn crypto rewards! Use my link: ${refData.referral_link}`;
      window.Telegram.WebApp.openTelegramLink(`https://t.me/share/url?url=${encodeURIComponent(refData.referral_link)}&text=${encodeURIComponent('Join me on Tasky!')}`);
    } else {
      handleCopy();
    }
  };
          ...
          <motion.div variants={containerVariants} initial="initial" animate="animate" className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <Card className="flex flex-col">
                ...
                <p className="text-lg font-bold">+{refData?.reward_per_referral || 200} & 1 Spin</p>
              </Card>
            </div>
            ...
            <Card>
              ...
              <ul className="space-y-3 text-sm text-ink-soft">
                ...
                <li className="flex gap-2">
                  <span className="text-success mt-0.5">•</span>
                  <span>You will instantly earn <strong className="text-ink">{refData?.reward_per_referral || 200} TASKY + 1 Spin</strong> for every valid referral!</span>
                </li>
              </ul>
