...
(async () => {
  try {
    ...
    // Start from index 1200 (since 0..1200 are already sent!)
    const START_INDEX = 1200;
    const targets = allTargets.slice(START_INDEX);

    console.log(`🚀 [TURBO ENGINE] Resuming Global Broadcast from index ${START_INDEX} (${targets.length} remaining users)...`);
    ...
    for (let i = 0; i < targets.length; i += BATCH_SIZE) {
      ...
      console.log(`[TURBO] Sent ${currentTotal} / ${allTargets.length} (Delivered: ${success}, Failed/Blocked: ${failed})`);
      await new Promise(r => setTimeout(r, 100)); // 100ms delay for maximum speed!
    }

    console.log(`🎉 FINISHED! ALL ${allTargets.length} USERS PROCESSED SUCCESSFULLY! Delivered: ${success}, Failed/Blocked: ${failed}`);
  } catch (err) {
    console.error('❌ Turbo Broadcast error:', err.message);
  } finally {
    process.exit();
  }
})();
