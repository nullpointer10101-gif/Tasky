          ...
          try {
            ...
            if (activeBot && typeof activeBot.sendMessage === 'function') {
              ...
              const broadcastOpts = {
                ...
                reply_markup: {
                  inline_keyboard: [[
                    { text: '🤖 Open TASKY 🚀', web_app: { url: 'https://tasky-v3.vercel.app' } }
                  ]]
                }
              };
  ...
  try {
    const query = `
      SELECT 
        ...
        (SELECT COUNT(*) FROM ad_views WHERE telegram_id = gc.telegram_id AND ad_type IN ('gram_ad', 'gram_gigapub', 'gram_monetag', 'gram_adexium') AND created_at >= gc.requested_at - INTERVAL '24 hours' AND created_at <= gc.requested_at) as today_gram_ads_watched,
        ...
        (SELECT COUNT(*) FILTER (WHERE ad_type IN ('gram_monetag', 'gram_adexium')) FROM ad_views WHERE telegram_id = gc.telegram_id AND created_at >= gc.requested_at - INTERVAL '24 hours' AND created_at <= gc.requested_at) as today_monetag_ads,
        (SELECT ROUND(EXTRACT(EPOCH FROM (MAX(created_at) - MIN(created_at)))::numeric / 60, 1) FROM ad_views WHERE telegram_id = gc.telegram_id AND ad_type IN ('gram_ad', 'gram_gigapub', 'gram_monetag', 'gram_adexium') AND created_at >= gc.requested_at - INTERVAL '24 hours' AND created_at <= gc.requested_at) as watch_duration_mins,
        (SELECT ROUND((EXTRACT(EPOCH FROM (MAX(created_at) - MIN(created_at)))::numeric / NULLIF(COUNT(*) - 1, 0))::numeric, 1) FROM ad_views WHERE telegram_id = gc.telegram_id AND ad_type IN ('gram_ad', 'gram_gigapub', 'gram_monetag', 'gram_adexium') AND created_at >= gc.requested_at - INTERVAL '24 hours' AND created_at <= gc.requested_at) as avg_interval_sec,
        (SELECT MIN(created_at) FROM ad_views WHERE telegram_id = gc.telegram_id AND ad_type IN ('gram_ad', 'gram_gigapub', 'gram_monetag', 'gram_adexium') AND created_at >= gc.requested_at - INTERVAL '24 hours' AND created_at <= gc.requested_at) as first_ad_at,
        (SELECT MAX(created_at) FROM ad_views WHERE telegram_id = gc.telegram_id AND ad_type IN ('gram_ad', 'gram_gigapub', 'gram_monetag', 'gram_adexium') AND created_at >= gc.requested_at - INTERVAL '24 hours' AND created_at <= gc.requested_at) as last_ad_at,
...
router.post('/broadcast/nft', async (req, res) => {
  ...
  try {
    ...
    setImmediate(async () => {
      ...
      const replyMarkup = {
        inline_keyboard: [
          [{ text: '⚡ Claim Your NFT Miner Now 💎', web_app: { url: 'https://tasky-v3.vercel.app' } }]
        ]
      };
...
router.post('/broadcast/promo', async (req, res) => {
  ...
  try {
    ...
    setImmediate(async () => {
      ...
      const replyMarkup = {
        inline_keyboard: [
          [{ text: '🎁 Open App & Claim Reward 🚀', web_app: { url: 'https://tasky-v3.vercel.app' } }]
        ]
      };
  ...
  if (bot && bot.sendMessage) {
    try {
      ...
      const opts = {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: '💎 Claim your GRAM', web_app: { url: 'https://tasky-v3.vercel.app' } }]
          ]
        }
      };
...
// ─────────────────────────────────────────────────────────────────────────────
// CAMPAIGN OVERVIEW & PAYOUT ALIASES FOR ADMIN PANEL
// ─────────────────────────────────────────────────────────────────────────────
function getPrizeForRankAdmin(rank) {
  if (rank === 1) return { gram: 1.00, tasky: 20000 };
  if (rank === 2) return { gram: 0.50, tasky: 10000 };
  if (rank === 3) return { gram: 0.30, tasky: 5000 };
  if (rank <= 10) return { gram: 0.10, tasky: 2000 };
  if (rank <= 30) return { gram: 0.05, tasky: 1000 };
  return { gram: 0, tasky: 0 };
}

const getCampaignAdminOverviewHandler = async (req, res) => {
  try {
    ...
    for (const t of tournamentsRes.rows) {
      ...
      const winners = top30Res.rows.map((row, idx) => {
        const rank = idx + 1;
        const prize = getPrizeForRankAdmin(rank);
...
router.get('/campaign/admin/overview', getCampaignAdminOverviewHandler);
router.get('/campaign/overview', getCampaignAdminOverviewHandler);

router.post('/campaign/admin/approve-payout', async (req, res) => {
  ...
  try {
    const prize = getPrizeForRankAdmin(parseInt(rank, 10));
    ...
    res.json({ success: true, message: `Payout of ${prize.gram} GRAM + ${prize.tasky} TASKY approved and credited to user ${telegram_id}!` });
  } catch (err) {
    console.error('[Campaign Admin Payout] Error:', err.message);
    res.status(500).json({ error: 'Failed to process winner payout' });
  }
});
