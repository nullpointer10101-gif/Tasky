...
`).then(() => {
  return pool.query("UPDATE nft_cards SET is_active = FALSE WHERE id IN (1, 2) OR rarity IN ('rare', 'legendary')");
  ...
  try {
    ...
    if (ownedCount >= maxAllowed) {
      ...
      return res.status(400).json({
        error: `Purchase limit reached! Every user is allowed a maximum of ${maxAllowed} purchases for ${nft.name} (You currently own ${ownedCount}/${maxAllowed}).`
      });
    }
    ...
    sendAdminBroadcast(
      `🛒 <b>🚀 NEW NFT MINER PURCHASE (${ownedCount + 1}/${maxAllowed})</b>\n\n` +
    ...
    const successMessage = `🎉 Successfully purchased ${nft.name}! (${ownedCount + 1}/${maxAllowed} owned). Check your Inventory tab to claim daily yield!`;
