async function check() {
  const html = await fetch('https://tasky-kohl-six.vercel.app').then(r => r.text());
  const m = html.match(/src="(\/assets\/index-[^"]+\.js)"/);
  if (!m) return console.log('no js match');
  const jsUrl = 'https://tasky-kohl-six.vercel.app' + m[1];
  ...
  const matches = js.match(/https:\/\/[a-zA-Z0-9_-]+\.onrender\.com/g);
  console.log('Render matches:', [...new Set(matches)]);
}
check();
